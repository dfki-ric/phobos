...
def create_pdf_from_urdf(urdf_path, name=None, only_gv=False, only_pdf=True):
    try:
        try:
            try:
                misc.execute_shell_command(" ".join(["urdf_to_graphviz", "-a", urdf_path, urdf_path[:-5]]), silent=True)
            except:
                misc.execute_shell_command(" ".join(["urdf_to_graphviz", urdf_path, urdf_path[:-5]]), silent=True)
        except:
            misc.execute_shell_command(" ".join(["urdf_to_graphiz", urdf_path]), silent=True)
            try:
                if name is not None:
                ...
                log.error("Failed to copy the files generated by urdf_to_graph(v)iz. Eventually urdf_to_graph(v)iz output "
                      "has been adapted. Nothing to worry about, if the PDF isn't too important for you...")
    except:
        log.error("ERROR: Couldn't create PDF as urdf_to_graphviz is not present")
    if only_gv:
        os.remove(urdf_path[:-4]+"pdf")
    if only_pdf:
        os.remove(urdf_path[:-4]+"gv")
